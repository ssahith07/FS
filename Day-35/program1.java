...
public class program1 {
    ...
    public static ArrayList<Integer> alive(int a[]) {
        Stack<Integer> st = new Stack<>();
        for (int s : a) {
            boolean zombie = true;
            while (!st.isEmpty() && s < 0 && st.peek() > 0) {
                int player = st.peek();
                if (Math.abs(s) > player) {
                    st.pop();
                    // break;
                    continue;
                } else if (Math.abs(s) == player) {
                    st.pop();
                    zombie = false;
                    break;
                } else {
        ...
        ArrayList<Integer> sol = new ArrayList<>(st);
        return sol;
